...
[passgen](https://github.com/defuse/passgen) (which was written in C). Unlike
`passgen`, `passgenr` has no defenses against side-channel attacks, so when you
generate passwords with `passgenr` you should be sure that nobody else is
running code on the same hardware (even as an unprivileged user). Adding
...
2<sup>ceil(lg(N))</sup> - 1] until it falls within the range [0, N-1].

## Security Audit Status

While its author is careful and prioritizes security, `passgenr` has not yet
been audited by a professional third-party. If you have a history of doing
